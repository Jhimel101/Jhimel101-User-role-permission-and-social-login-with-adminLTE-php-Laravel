@extends('master')

@section('content')
@include('header2')
  
    <div class="container"  style="width: 60; margin-left:100px">
     
            <div class="container-fluid">
                <form class="d-flex">
                    <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search">
                    <button class="btn btn-outline-success">Search</button>
                </form>
            </div>
        
    </div>

    <div class="container-fluid" style="border-color: lightgrey;width: 95%">


        <div class="card" style="background-color:#f7f7f7; padding-bottom: 500px; padding-top:50px;
            <br>
            <div class="card-body">
                <div class="container">
                    <div class="row align-items-start">
                        <div class="col" >
                            <div class="card" style="width: 16rem">
                                <br>
                                <br>
                                <img src="data:image/png;base64,"
                                    class="card-img-top" alt="...">
                                    <br>
                                    <br>
                                <form action="/action_page.php">
                                    <input type="file" id="myFile" name="filename">
                                </form>
                                <br>
                                <div class="card-body" style="background-color:#4f4cae">
                                    <h5 class="card-title" style="color:white; text-align:center">Admin Profile</h5>
                                </div>
                                <div class="list-group">
                                  
                                    <button type="button" class="list-group-item list-group-item-action"
                                        style="background-color:#4f4cae;color:white; text-align:center">Registration Page</button>
                                    <button type="button" class="list-group-item list-group-item-action"
                                        style="background-color:#4f4cae;color:white; text-align:center">Login Page
                                        </button>
                                   
                                    <button type="button" class="list-group-item list-group-item-action"
                                        style="background-color:#4f4cae;color:white; text-align:center">Appointments
                                        </button>
                                        <button type="button" class="list-group-item list-group-item-action"
                                        style="background-color:#4f4cae;color:white; text-align:center">Product Orders
                                        </button>
                                    <button type="button" class="list-group-item list-group-item-action"
                                        style="background-color:#4f4cae;color:white; text-align:center">Change Booking
                                        </button>
                                        <button type="button" class="list-group-item list-group-item-action"
                                        style="background-color:#4f4cae;color:white; text-align:center">Update System
                                        </button>
                                </div>
                            </div>
                        </div>
                        <div class="col">
                            <div class="card text-center" style="width: 18rem">
                                <div class="card-body" style="background-color: white">
                                    <br>
                                    <h5 class="card-title" style="color:black">Vendors</h5>
<br>
                                    <a href="#" class="btn btn-primary" style="background-color: #4f4cae"   >Add User</a>
                                    <a href="#" class="btn btn-primary"  style="background-color: #4f4cae" >   Remove User</a>
                                </div>
                            </div>
                        </div>
                        <br>
                        <br>
                        <div class="card text-center" style="width: 18rem; background-color:white">
                            <div class="card-body">
                                <br>
                                
                                <h5 class="card-title" style="color:black">Customers</h5>
<br>
                                <a href="#" class="btn btn-primary"  style="background-color: #4f4cae" >Add user</a>
                                <a href="#" class="btn btn-primary" style="background-color: #4f4cae" >Remove User</a>
                            </div>
                        </div>
                    </div>
                    
                    <table class="table" style= "width:53rem; margin-top:-310px; margin-left:300px;color:black;background-color: white" >
                        
                        <thead>
                            <tr> 
                                <th scope="col"></th>
                                <th scope="col"></th>
                                <th scope="col"></th>
                                <th scope="col"></th>
                                <th scope="col">User Details</th>
                                <th scope="col"></th>
                                <th scope="col"></th>
                            </tr>
                    
                            <tr> 
                                <th scope="col">#</th>
                                <th scope="col">Username</th>
                                <th scope="col">Name</th>
                                <th scope="col">Type of User</th>
                                <th scope="col">Email address</th>
                                <th scope="col">Address</th>
                                <th scope="col">Contact Number</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <th scope="row">1</th>
                                <td>Farhana</td>
                                <td>Farhana Khan</td>
                                <td>Vendor</td>
                                <td>farhana.khan@northsouth.edu</td>
                                <td>efeefef kgvrmvr</td>
                                <td>fwfegg</td>
                            </tr>
                            <tr>
                                <th scope="row">2</th>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                            </tr>
                            <tr>
                                <th scope="row">3</th>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                            </tr>
                            <tr>
                                <th scope="row">4</th>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                            </tr>
                            <tr>
                                <th scope="row">5</th>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                            </tr>
                            <tr>
                                <th scope="row">6</th>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                            </tr>
                            <tr>
                                <th scope="row">7</th>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                            </tr>
                            <tr>
                                <th scope="row">8</th>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                                <td>xx</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
    </div>
       
    </div>
</div>
</div>
